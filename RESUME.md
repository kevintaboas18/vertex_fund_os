...
| Macroeconómico (derecha) | **Los doce últimos datos de alto impacto que salieron** —la lista de alto impacto es la que dio Kevin el 27/08/2026 (`_MACRO_TABLA`) más el coste laboral del BLS y las expectativas de inflación de Michigan, que él no nombró; y lo que FMP marque `High` entra aunque no esté en ella, sin color porque no sabemos su dirección. **Sólo alto impacto**: lo de impacto medio o bajo no sale ni aquí ni en «Próximos datos», y si en la ventana sólo hay tres, salen tres—, con sus tres columnas —salió · esperado · anterior—, un botón «Explícame qué está pasando» que traduce la sorpresa a lo que significa para la economía, para la gente y para cada uno de los once sectores, y debajo **los próximos datos con su hora ET**. Una fila por indicador **y por corte**: el IPC mensual y el interanual son dos datos y salen los dos, con el que cita el mercado delante. Los rótulos que FMP repite para la misma serie —«Inflation Rate YoY» y «CPI YoY»— se colapsan a uno para no gastar dos de los ocho huecos en decir lo mismo. |
