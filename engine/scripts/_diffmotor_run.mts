...
// `r6` aplicado en PROFUNDIDAD. Sin esto, todo escalar que no pasara por `r6`
// —`score`, `mfe`, `openInterest`…— llegaba al comparador como `null`, porque
// `JSON.stringify(NaN)` y `JSON.stringify(Infinity)` son `null`. Un `NaN` de su
// archivo se leía entonces como "sin dato" y tapaba la divergencia real.
const d6 = (x: any): any =>
...
const validation = C.validation.map((c: any) => {
  try {
    ...
    return d6({ score: r.score, hr: r.hitRate, sp: r.speed, whr: r.weightedHitRate,
...
const levels = C.levels.map((c: any) => {
  try {
    ...
    const m = (a: any[]) => a.map((l: any) => [r6(l.price), l.kind, r6(l.strength),
      ...
      l.flipped, l.why]);
    return d6({ spot: r.spot, sup: m(r.supports), res: m(r.resistances),
...
const structure = C.structure.map((rows: any) => {
  try {
    ...
    return d6({ score: r.score, notional: r.notional, volOI: r.volOI,
