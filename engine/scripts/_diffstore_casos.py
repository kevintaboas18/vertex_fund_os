...
# El port de `store.ts` es LITERAL: cero divergencias declaradas. Los tres casos
# que su archivo maneja mal —ticker que no da nombre, dedupe sin id, fila
# corrupta en disco— están portados tal cual y por eso aparecen aquí como casos
# IDÉNTICOS, no como guardas. Las guardas viven en `wbj/tito/borde.py`, que es
# el borde de Vertex y no forma parte de su motor.
#
# El comparador sigue admitiendo `divergencia=` a propósito: si algún día hace
# falta una, tiene que declararse y tiene que producir de verdad una diferencia.
...
for t in ("demo", " demo ", "DEMO", "brk.b", "brk/b", "!!!", "", "a"*70,
          "../../ETC/X", "a b", "ñ"):
    # Los que sanean a nada acaban TODOS en el mismo `.json`, en su archivo y en
    # el port. El rechazo lo hace `borde.ticker_valido` en el endpoint, que es
    # donde su pipeline de Next lo tiene: aquí no hay diferencia que declarar.
    c(nombre=f"save ticker={t!r}", op="save", ticker=t, rows=[fr(1,T(30))])

# ── save: valores raros ──
c(nombre="save id ausente (0)", op="save", ticker="Z1",
